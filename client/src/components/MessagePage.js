...
export default function MessagePage() {
  ...
  const handelUploadImageVideoOpen = () => {
  ...
  //console.log("message iamge",message)
    ...
    <div style={{ backgroundImage: `url(${backgroundImage})` }} className='bg-no-repeat bg-cover'>
        ...
        <div className='flex flex-col gap-2 py-2 mx-2' ref={currentMessage}>
          {
            allMessage.map((msg, index) => {
              return (
                <div
                  key={msg._id || index}
                  className={`p-1 py-1 rounded w-fit max-w-[280px] md:max-w-sm lg:max-w-md  ${user._id === msg.msgByUserId ? "ml-auto bg-teal-200" : "bg-white"}`}
                >
      ...
      <section className='h-16 bg-white flex items-center px-4'>
        <div className='relative'>
          <button onClick={handelUploadImageVideoOpen} className='flex justify-center items-center w-11 h-11 rounded-full hover:bg-primary hover:text-white'>
            <FiPlus size={20} />
          </button>
          {/* video and image */}
        ...
        {/**inputbox */}
        <form className='h-full w-full flex gap-6' onSubmit={handelSendMessage}>
          <input
            type='text'
